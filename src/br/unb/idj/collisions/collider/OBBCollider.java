...
public class OBBCollider extends Collider {
	public OBBCollider(CollidingObject owner) {
		if (owner != null)
	...
	public void paint(Graphics2D g2d) {
		...
		if (isDrawShadow()) {
			...
			g2d.fillRect((int) bounds.getCenterX(), bounds.y, dx + 1, bounds.height);
			g2d.setColor(Colors.SHADOW_BORDER);
			g2d.drawRect(dx - bounds.width / 2, bounds.y, bounds.width, bounds.height - 1);
		}
	...
	public boolean hitsPoint(Vector2 point) {
		try {
			Rectangle bounds = getTextureBounds();
			if (bounds != null) {
				Point2D src = new Point2D.Float(point.x, point.y);
				src = getTransform().inverseTransform(src, null);
				return bounds.contains(src);
			}
